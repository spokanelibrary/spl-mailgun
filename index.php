...
class SPL_Mailgun_Newsletter {
    ...
    switch ( $params['format'] ) {
      ...
      case 'toc':
        ...
        if ( in_array('excerpt', $params) ) {
          ...
          $excerpt .= ' <a class="" href="'.get_the_permalink($q->post->ID).'"><b>Continued</b> <small>&rarr;</small></a>';
        ...
        if ( !empty($posts) ) {
          ...
          if ( isset($params['link_button']) ) {
            $html .= '<a class="btn btn-large btn-success" href="'.get_the_permalink($q->post->ID).'">Read this issue of Library News &rarr;</a>';
          }
          $html .= '</div>';
          $html .= '</div>';
        }
  ...
  function getNewsletterCmbMetaBoxes( $meta_boxes ) {
    ...
    // TEMPLATE
    $tmpls = array();
    $tmpls[] = array('name'=>'Default', 'value'=>'default'); 
      ...
      foreach( $templates as $template ) {
        if ( !is_dir(plugin_dir_path(__FILE__).'templates/'.$template) ) {
          if ( stristr($template, 'single') ) {
            $tmpls[] = array('name'=>$template, 'value'=>$template);
          }
          //$tmpl .= '<option value="'.$template.'">';
          //$tmpl .= str_replace(array('.php', '_'), array('',' '), $template);
          //$tmpl .='</option>';
